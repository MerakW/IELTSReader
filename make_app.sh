...
swift build --cache-path "$ROOT_DIR/.build/swiftpm-cache"

rm -rf "$APP_DIR"
mkdir -p "$MACOS_DIR" "$RESOURCES_DIR"
cp "$ROOT_DIR/.build/debug/IELTSReader" "$MACOS_DIR/IELTSReader"
...
if [[ -x "$ACTOOL" && -d "$ROOT_DIR/Resources/IELTSReader.icon" ]]; then
  "$ACTOOL" \
    ...
    --output-partial-info-plist "$CONTENTS_DIR/AssetCatalogInfo.plist" \
